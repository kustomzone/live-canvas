...
- 🌳 **画布是无限延伸的树** —— 每次点击都生成一个子节点；整棵探索树会被持久
  化、可分享、可回放。
- ⏳ **看着它"思考"** —— 点击瞬间节点就已落盘且可链接，随后标题/图说/画面
  描述**逐字流式打出**；把链接发给别人，另一台设备也能看着同一份内容实时填充。
...
- 🖱️ **点击式探索**：在节点图上任意位置 **长按 1 秒**。后端推断你点击位置的
  语义标签、判断是否需要联网搜索、最终生成一个子节点。空间 + 语义双重去重，
  再次点击同一区域会直接跳进去。
- ⏳ **生成中节点即时可链接、流式打字**：点击瞬间子节点就以**最终 id 落盘**、
  父热点立刻指向它 —— 因此**生成期间就能分享 / 在任意设备打开**。其标题、图说、
  画面 prompt 会通过 SSE **逐 token 流式打出**，目录里显示一行 **spinner**，
  刷新或换设备打开会**从磁盘快照续上流**。生成失败则自动删除半成品节点。
- 🌫️ **图片渐进式加载**：每张 PNG 生成 模糊 → 缩略 → 中等 → 原图 多档变体
  （sharp）。画廊卡片由模糊渐显，画布就绪后切到原图 —— 没有裂图闪烁、首屏更快。
- 🖼️ **横屏 / 竖屏画布**：按画布选择方向（移动端竖屏视口默认竖屏）；画廊可按
  **全部 / 横屏 / 竖屏** 过滤，且筛选条件与 URL 同步。
- ⚡ **节点级并发**：同一父节点最多 **4 个不同位置同时生成**（可配置）。每个
  进行中的点击都会在热点上流式展示阶段（`推断标签…` → `搜索网络…` →
  `生成图像…`）。达到上限后光标变为 ⌛。
- 📖 **百科图鉴风格**：planner 产出 150–220 字的图说和 20–40 处图内文字注释——
  像在读一本细致标注的儿童百科图鉴。过长图说默认夹断为 2 行，配
  **查看更多 / 收起** 开关。
...
- 🔁 **断线可续的 SSE**：Last-Event-ID 重放 + 按 job 的快照续接 —— 生成中途断
  连或刷新页面，重连后会补齐所有错过的事件，包括进行中的流式打字。
...
- 默认会在全屏下隐藏"长按 1 秒"的提示文案，但长按本身仍然可用。

## 🧹 清理本地状态

```bash
npm run clean:data    # 重置 server/data（所有 canvas）
npm run clean:dist    # 重置 web/dist
