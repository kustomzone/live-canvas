...
(function () {
  ...
  function render() {
    ...
    requestAnimationFrame(function () {
      ...
      if (!sr.width || !sr.height) return;
      // Label font size from the DISPLAYED image's shortest edge so labels
      // stay proportional across viewport sizes / orientations.
      (function sizeLabels() {
        var iw = (node.image_w) || (imgEl && imgEl.naturalWidth);
        var ih = (node.image_h) || (imgEl && imgEl.naturalHeight);
        var wPct = 100, hPct = 100;
        if (iw && ih) {
          var stageAspect = sr.width / sr.height, imgAspect = iw / ih;
          if (imgAspect > stageAspect) { hPct = (stageAspect / imgAspect) * 100; }
          else if (imgAspect < stageAspect) { wPct = (imgAspect / stageAspect) * 100; }
        }
        var dispW = (wPct / 100) * sr.width, dispH = (hPct / 100) * sr.height;
        var shortEdge = Math.min(dispW, dispH);
        if (!(shortEdge > 0)) return;
        var fontPx = Math.max(8, Math.min(13, Math.round(shortEdge * 0.022)));
        for (var key in cardEls) { if (cardEls[key]) cardEls[key].style.fontSize = fontPx + 'px'; }
      })();
      layouts.forEach(function (lay) {
